...
from content.models import CategoryModel, AlbumModel, PhotoModel
import logging
...
def WriteLoctionJsonToDatabase(request):
    ...
    for key in l.keys():
        ...
        for city in l[key]:
            ...
            c.save() 
    return HttpResponse('initProvince  succeed!')
...
def initContentIndex():
    ...
    album = AlbumModel.object.filter(title ='title39')[0]
    
    for i in range(40):
        PhotoModel.object.create(profile=userProfile, album=album, title='title%d'%i, 
                                  url = 'www.baidu.com', description='description%d'%i, parameter='parameter%d'%i)
